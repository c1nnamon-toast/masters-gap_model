...
Runtime note
------------
Occlusion is slower than Integrated Gradients.  Rubsheet_3to1 images are
1602×534 px.  The default parameters below give roughly 48×16 ≈ 768 forward
passes per image, which is manageable on a GPU.  Increase STRIDE or
...
# patch=34 (≈ 1/16 of height) and stride=17 (half-patch) yield
# roughly 95×32 ≈ 3 040 forward passes per image on a wide image, or
# use patch=67 / stride=34 for ~24×8 ≈ 192 faster passes.
